...
def from_xml(xml):
    coverage, branches = {}, {}
    for _class in getattr(xml, 'iter', 'findall')('class'):
        # extract file name
        ...
        lines = [None]*(max([int(line.attrib['number']) for line in getattr(_class, 'iter', 'findall')('line')] or [0])+1)
        if len(lines) == 1:
            continue

        for line in getattr(_class, 'iter', 'findall')('line'):
